...
export default function MovieCreate() {
    ...
    const validationSchema = Yup.object().shape({
        ...
        original_title: Yup.string(), // optional
        ...
        director_id: Yup.number()
            .min(1, 'Director ID must be > 0')
            .required('Director is required'),
        country_id: Yup.number()
            .min(1, 'Country ID must be > 0')
    ...
    const handleSubmit = async (values: MovieFormValues, { setSubmitting, resetForm }: any) => {
        try {
            ...
            const payload = {
                ...
                actors: JSON.stringify(values.actors.filter(a => a.name || a.character))
            };
